...
import { Menu, Search, Edit2, Trash2, Plus, LayoutGrid, List, ImageIcon, RefreshCw, ChevronLeft, ChevronRight, ChevronDown, Tag } from "lucide-react";
        ...
        <div className="p-6 max-w-7xl mx-auto w-full flex-1 flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-6 mb-8">
            ...
            <button 
              ...
              className="flex items-center justify-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white bg-black hover:bg-zinc-800 shadow-lg transition-colors w-full sm:w-auto shrink-0"
            >
              <Plus size={18} />
              Add Product
            </button>
          ...
          <div className={`flex flex-col md:flex-row gap-4 mb-8 p-4 md:p-5 rounded-xl border ${theme.border} ${theme.cardBg} shadow-sm`}>
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto items-start md:items-center">
              ...
              {/* View Toggle */}
              <div className={`flex w-full md:w-auto bg-zinc-50 p-1.5 rounded-md border ${theme.border} shrink-0`}>
                <button onClick={() => setViewMode("table")}
                  className={`flex-1 md:flex-none px-4 py-2 sm:px-3 sm:py-1.5 rounded-md flex items-center justify-center transition-all ${viewMode === "table" ? "bg-white shadow-sm text-black" : "text-slate-500 hover:text-slate-900"}`}
                  title="Table View" >
                  <List size={18} />
                </button>
                <button onClick={() => setViewMode("card")}
                  className={`flex-1 md:flex-none px-4 py-2 sm:px-3 sm:py-1.5 rounded-md flex items-center justify-center transition-all ${viewMode === "card" ? "bg-white shadow-sm text-black" : "text-slate-500 hover:text-slate-900"}`}
                  title="Card View" >
                  <LayoutGrid size={18} />
                </button>
              </div>
          ...
          <div className={`${theme.cardBg} border ${theme.border} rounded-xl shadow-sm overflow-hidden flex flex-col`}>
              ...
              <div className={`overflow-y-auto ${customScrollbar} flex-1 p-3 sm:p-5 bg-slate-50/50`}>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5">
                  {currentProducts.map((product) => {
                    ...
                    return (
                      <div key={id} className="bg-white rounded-xl border border-zinc-200 p-4 shadow-sm hover:shadow-md transition-shadow flex flex-col group">
                        
                        {/* Image Section */}
                        <div className="w-full aspect-4/3 rounded-lg bg-zinc-50 border border-zinc-100 flex items-center justify-center text-zinc-300 relative overflow-hidden mb-4 shrink-0">
                          {primaryImage ? (
                            <img src={primaryImage} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 ease-out" />
                          ) : (
                            <div className="flex flex-col items-center gap-2">
                              <ImageIcon size={32} className="opacity-50" />
                              <span className="text-xs font-bold uppercase tracking-wider text-zinc-400">{initials}</span>
                            </div>
                          )}
                          
                          {/* Discount Overlay */}
                          {hasDiscount && (
                            <div className="absolute top-2 right-2 bg-rose-500 text-white text-[10px] font-bold px-2 py-1 rounded shadow-sm">
                              {discountBadgeText}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1 flex flex-col">
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 truncate">{product.category}</span>
                            <span className="text-[10px] font-semibold text-slate-400 capitalize truncate">{product.brand}</span>
                          </div>
                          
                          <h3 className="font-bold text-slate-900 text-base leading-tight mb-2 line-clamp-2" title={product.name}>
                            {product.name}
                          </h3>

                          {/* Pricing block pinned to bottom of this section */}
                          <div className="mt-auto pt-2 flex items-end gap-2">
                            <span className="font-bold text-slate-900 text-xl">₹{finalPrice.toFixed(2)}</span>
                            {hasDiscount && (
                              <span className="text-sm font-medium text-slate-400 line-through mb-0.75">₹{product.price.toFixed(2)}</span>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="pt-4 mt-4 border-t border-zinc-100 flex items-center justify-between">
                          <div className="flex flex-col items-start gap-1">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider border ${stockStatus.classes}`}>
                              {stockStatus.label}
                            </span>
                          ...
                          <div className="flex items-center gap-2">
                            <button onClick={() => navigate(`/products/update/${id}`)} className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md bg-zinc-50 border border-zinc-200 text-slate-600 hover:bg-zinc-100 hover:text-slate-900 active:scale-95 transition-all duration-150">
                              <Edit2 size={14} />
                            </button>
                            <button onClick={() => deleteProduct(id)} className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md bg-rose-50 border border-rose-100 text-rose-600 hover:bg-rose-100 hover:text-rose-700 active:scale-95 transition-all duration-150">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </div>
            ...
            {/* Empty State properly placed outside the conditional view logic */}
